...
class Stock {
    ...
    */
    ...
    // [Follow up on Q above] Y. Perhaps we should consider that the observer interface here is _only_ interested
    // in the change of this property. If there are more properties of the subject that interest observers too,
    // there should be another observer interface.
    private var value: Double by Delegates.observable(0.0) { _, _, newValue ->
        listOfListeners.forEach { it.onStockChange(newValue) }
    }
